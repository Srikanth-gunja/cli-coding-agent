...
import os
from .response import TokenUsage, StreamEventType, StreamEvent, TextDelta
        ...
        for attempt in range(self._max_tries + 1):
            try:
                ...
                if attempt < self._max_tries:
                    ...
                    yield StreamEvent(
                        type=StreamEventType.ERROR, error=f"Rate Limit exceede :{ae}"
                    )
                    return
            except APIError as ae:
                yield StreamEvent(type=StreamEventType.ERROR, error=f"API Error:{ae}")
                return
